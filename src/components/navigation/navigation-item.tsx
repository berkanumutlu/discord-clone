    ...
    return (
        <ActionTooltip side="right" align="center" label={name}>
            <button className="group relative flex items-center" onClick={onClick}>
                <div className={cn(
                    "hidden group-hover:block absolute left-0 w-[4px] bg-primary rounded-r-full transation-all",
                    params?.serverId !== id && "group-hover:h-[20px]",
                    params?.serverId === id ? "block h-[36px]" : "h-[8px]"
                )} />
                <div className={cn(
                    "group mx-3 w-[48px] h-[48px] relative flex rounded-[24px] group-hover:rounded-[16px] transition-all overflow-hidden",
                    params?.serverId === id && "bg-primary/10 text-primary rounded-[16px] group-hover:rounded-[8px]"
                )}>
